            ...
            with gr.TabItem("🎙️ Transkription"):
                with gr.Row():
                    with gr.Column():
                        ...
                        # --- PROVIDER SELECTION ---
                        with gr.Row():
                            t_prov = gr.Radio(["Gladia", "Deepgram", "AssemblyAI", "Mistral", "Scaleway", "Groq"], value="Gladia", label="Engine", scale=2)
                            t_load_all = gr.Button("🌍 Alle", scale=0, size="sm")
                            
                        # t_model ist jetzt sichtbar für Whisper und Deepgram/AA
                        ...
                        # DEEPGRAM OPTIONS
                        deepgram_opts = gr.Accordion("⚙️ Deepgram Optionen", open=True, visible=False)
                        with deepgram_opts:
                            t_dg_lang = gr.Dropdown([("Deutsch", "de"), ("Englisch", "en")], value="de", label="Sprache")
                            t_dg_diar = gr.Checkbox(True, label="🎭 Sprecher erkennen (Diarization)")

                        # ASSEMBLYAI OPTIONS
                        assemblyai_opts = gr.Accordion("⚙️ AssemblyAI Optionen", open=True, visible=False)
                        with assemblyai_opts:
                            t_aa_lang = gr.Dropdown([("Deutsch", "de"), ("Englisch", "en")], value="de", label="Sprache")
                            t_aa_diar = gr.Checkbox(True, label="🎭 Sprecher erkennen (Labels A, B, ...)")
                        ...
                        t_prov.change(
                            ...
                            t_prov, 
                            [t_badge, t_model, gladia_opts, whisper_opts, deepgram_opts, assemblyai_opts]
                        )
                        t_load_all.click(
                            lambda p: update_t_ui(p, force_all=True), 
                            t_prov, 
                            [t_badge, t_model, gladia_opts, whisper_opts, deepgram_opts, assemblyai_opts]
                        )
                ...
                def sync_languages(lang_value):
                    # NEU: Synchronisiere Gladia/Whisper/Deepgram/AA Sprachen
                    return (
                        lang_value, # t_lang (Gladia)
                        lang_value, # w_lang (Whisper)
                        lang_value, # t_dg_lang (Deepgram)
                        lang_value  # t_aa_lang (AssemblyAI)
                    )

                # Sprachen synchronisieren
                t_lang.change(sync_languages, t_lang, [w_lang, t_dg_lang, t_aa_lang])
                w_lang.change(sync_languages, w_lang, [t_lang, t_dg_lang, t_aa_lang])
                t_dg_lang.change(sync_languages, t_dg_lang, [t_lang, w_lang, t_aa_lang])
                t_aa_lang.change(sync_languages, t_aa_lang, [t_lang, w_lang, t_dg_lang])
                ...
                t_btn.click(
                    ...
                    inputs=[
                        ...
                        t_dg_lang, t_dg_diar, t_aa_lang, t_aa_diar # NEUE PARAMETER
                    ], 
