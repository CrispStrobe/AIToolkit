...
def update_t_ui(prov, force_all=False):
    # 1. Handle Badge Formatting (Fixing White-on-White)
    raw_html = get_compliance_html(prov)
    # Wrap in a styled div to ensure visibility in both dark/light modes
    styled_badge = f"""
    <div style="background-color: #2b2b2b; color: #e0e0e0; padding: 8px 12px; border-radius: 6px; border: 1px solid #444; margin-bottom: 10px; font-size: 0.9em;">
        {raw_html}
    </div>
    """

    # 2. Handle Models
    # Show model dropdown for Deepgram, AssemblyAI, and Whisper-based (Mistral, Scaleway, Groq)
    ...
    if show_model_dropdown:
        ...
        # Create (Label, Value) pairs. If raw_list is strings, map s->s
        choices = [(m, m) for m in raw_list]
        
        # Optional: Apply your filtering logic here if 'get_filtered_model_choices' exists
    ...
    # 3. Return Updates
    # Output 1: Badge HTML
    # Output 2: Model Dropdown (Visibility & Choices)
    # Output 3: Whisper Options Group (Visibility)
    return (
        styled_badge,
                    ...
                    with gr.Column():
                        ...
                        with gr.Group():
                            with gr.Row():
                                ...
                                # Global Diarization Checkbox
                                t_diar = gr.Checkbox(True, label="🎭 Sprecher erkennen", scale=1)

                        # --- SETTINGS ACCORDION ---
                        # Fixed: Single accordion for all settings to prevent empty UI blocks
                        with gr.Accordion("⚙️ Einstellungen", open=False):
                            
                            # 1. Compliance Badge (Now inside settings, styled)
                            ...
                            # 4. Whisper/Technical Specifics (Group, not Accordion)
                            # This group is hidden/shown by update_t_ui based on provider
